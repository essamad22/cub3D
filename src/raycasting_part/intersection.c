...
/*   Updated: 2024/02/29 20:57:54 by aakhtab          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ray h_intersec(t_player p, double angle, t_par par)
{
    ...
    if ((!ray_utils.ray_right && x_step > 0) || (ray_utils.ray_right && x_step < 0))
        x_step *= -1;
    if (!ray_utils.ray_down)
        p_a[1] -= 1.0;
    while (p_a[0] >= 0 && p_a[0] <= par.width * TILE_SIZE && p_a[1] >= 0 && p_a[1] <= par.height * TILE_SIZE)
...
t_ray v_intersec(t_player p, double angle, t_par par)
{
    ...
    if (!ray_utils.ray_right)
        point_a[0] -= 1.0;
    while (point_a[0] >= 0 && point_a[0] <= par.width * TILE_SIZE && point_a[1] >= 0 &&
           point_a[1] <= par.height * TILE_SIZE)
